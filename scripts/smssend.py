...
cliparser.add_argument("--to", metavar="PHONENUMBER", help="Telephone number where send sms message (required)",
                       required=True)
cliparser.add_argument("--message", metavar="MESSAGE",
                       help="Read the message from message (a file) instead of stdin (optional)")
...
cliparser.add_argument("--http_timeout", metavar="VALUE", help="Timeout for http connection (optional, default is 10)",
                       default=10)
...
servicecodes = {
    100: "Сообщение принято к отправке."
         "На следующих строчках вы найдете идентификаторы отправленных сообщений в том же порядке,"
         "в котором вы указали номера, на которых совершалась отправка.",
...
def main():
    ...
    if service_result is not None and int(service_result[0]) == 100:
        ...
        sys.exit(0)
    if service_result is not None and int(service_result[0]) != 100:
        show_debug_messages("smssend[debug]: Unable send sms message to {} when service has returned code: {} ".format(
            cliargs.to,
